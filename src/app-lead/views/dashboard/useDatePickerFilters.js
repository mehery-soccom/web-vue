  ...
  const customPlugin = fp => ({
    onReady() {
      ...
      container.querySelectorAll('button').forEach(btn => {
        btn.addEventListener('click', () => {
          fp.set('minDate', null);
          fp.set('maxDate', new Date().setHours(23, 59, 59, 999));
    ...
    onChange(selectedDates) {
      ...
      if (selectedDates.length === 1) {
        ...
        min.setDate(startDate.getDate() - 30); // 30 days behind

        const max = new Date(startDate);
        max.setDate(startDate.getDate() + 30); // 30 days ahead

        fp.set('minDate', min);
        fp.set('maxDate', max > tonight ? tonight : max);
      } else {
        fp.set('minDate', null);
        fp.set('maxDate', tonight);
      }
    }
  })
