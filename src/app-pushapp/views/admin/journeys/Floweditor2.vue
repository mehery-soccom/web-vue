      ...
      <aside v-if="inspectedNode" class="inspector">
        ...
        <div class="inspector-body">
          ...
          <template v-if="inspectedNode.data.code === 'TRIGGER'">
            <label class="field-label">App event</label>
            <select class="field-input" :value="inspectedNode.data.attrs.appevent"
              @change="setNodeAttr('appevent', $event.target.value);
              setNodeAttr('name',optionCache[inspectedNode.id]?.eventOptions?.find(o => o.value === $event.target.value)?.title || '')"
            >
              <option value="" disabled>
                {{ optionCache[inspectedNode.id]?.loadingEvents ? 'Loading…' : 'Select event…'}}
              </option>
              <option v-for="opt in optionCache[inspectedNode.id]?.eventOptions || []" :key="opt.value" :value="opt.value">
                {{ opt.title }}
              </option>
            </select>

            <!-- <p class="field-hint"> Output: <code>Start</code></p> -->
          </template>
          ...
          <template v-else-if="inspectedNode.data.code === 'ACTOR'">
            ...
            <select
              class="field-input"
              :value="inspectedNode.data.attrs.channelType"
              @change="onChannelTypeChange(inspectedNode.id, $event.target.value)"
            >
              <option v-for="opt in CHANNEL_TYPE_OPTIONS" :key="opt.value" :value="opt.value">{{ opt.title }}</option>
            </select>

            <label class="field-label">
              {{ inspectedNode.data.attrs.channelType === 'SEND_MESSAGE' ? 'WhatsApp channel' : 'App' }}
            </label>
            <select
              class="field-input"
              :value="inspectedNode.data.attrs.channelId"
              @change="onChannelIdChange(inspectedNode.id, inspectedNode.data.attrs.channelType, $event.target.value)"
            >
              <option value="" disabled>
                {{ optionCache[inspectedNode.id]?.loadingChannel ? 'Loading…' : 'Select…' }}
              </option>
              <option v-for="opt in optionCache[inspectedNode.id]?.channelOptions || []" :key="opt.value" :value="opt.value">
                {{ opt.title }}
              </option>
            </select>
            ...
            <select
              class="field-input"
              :value="inspectedNode.data.attrs.template?.code"
              :disabled="!inspectedNode.data.attrs.channelId"
              @change="onTemplateChange(inspectedNode.id, $event.target.value)"
            >
              <option value="" disabled>
                {{ optionCache[inspectedNode.id]?.loadingTemplate ? 'Loading…' : 'Select…' }}
              </option>
              <option v-for="opt in optionCache[inspectedNode.id]?.templateOptions || []" :key="opt.value" :value="opt.value">
                {{ opt.title }}
              </option>
            </select>
            ...
            <div class="listeners-block">
              <div class="listeners-head">
                ...
                <button class="add-listener-btn" @click="addListener">+ Add</button>
              </div>
              <!-- <p v-if="!inspectedNode.data.attrs.listeners?.length" class="field-hint">
                No listeners yet — this node has no outputs until you add one (e.g. a button click).
              </p> -->
              <p v-if="!inspectedNode.data.attrs.listeners?.length" class="field-hint">
                No listeners configured.<br>
                Output <code>Continue flow</code> is added automatically.
              </p>
              <!-- <div class="listener-row fixed-listener">
                <span class="field-input small">
                  Continue flow
                </span>
              </div> -->
              <div v-for="(l, i) in inspectedNode.data.attrs.listeners" :key="l.id" class="listener-row">
                <select class="field-input small" :value="l.type || 'text'" @change="setListenerType(i, $event.target.value)">
                ...
                <input
                  ...
                  :value="l.type === 'text' ? l.text : l.code"
                  @input="updateListenerText(i, $event.target.value)"
                  :placeholder="l.type === 'text' ? 'Text' : 'Code'"
                />
                <button class="remove-listener-btn" @click="removeListener(i)">✕</button>
              </div>
            </div>
          </template>
          ...
          <template v-else-if="inspectedNode.data.code === 'EXPECTATION'">
            <label class="field-label">Expected Event</label>
            <select class="field-input" :value="inspectedNode.data.attrs.appevent"
              @change="setNodeAttr('appevent', $event.target.value);
              setNodeAttr('name',optionCache[inspectedNode.id]?.eventOptions?.find(o => o.value === $event.target.value)?.title || '')"
            >
            ...
            <input
              type="number"
              class="field-input"
              :value="inspectedNode.data.attrs.window?.value"
              @input="setNodeAttr('window.value', Number($event.target.value))"
            />
            <label class="field-label">Time units</label>
            <select
              class="field-input"
              :value="inspectedNode.data.attrs.window?.unit"
              @change="setNodeAttr('window.unit', $event.target.value)"
            >
            ...
            <label class="field-label">Action (Fire and Forget - No Listeners)</label>
            <select
              class="field-input"
              :value="inspectedNode.data.attrs.channelType"
              @change="onChannelTypeChange(inspectedNode.id, $event.target.value)"
            ...
            <select
              ...
              :value="inspectedNode.data.attrs.template?.code"
              :disabled="!inspectedNode.data.attrs.channelId"
              @change="setNodeAttr('template.code', $event.target.value);
              setNodeAttr('template.name',optionCache[inspectedNode.id]?.templateOptions?.find( o => o.value === $event.target.value)?.title || '')"
            >
              <option value="" disabled>
                {{ optionCache[inspectedNode.id]?.loadingTemplate ? 'Loading…' : 'Select…' }}
              </option>
          ...
          <template v-else-if="inspectedNode.data.code === 'END'">
            ...
            <select class="field-input" :value="inspectedNode.data.attrs.status" @change="setNodeAttr('status', $event.target.value)">
...
/* ══ INSPECTOR ══ */
.inspector {
  width: 290px; flex-shrink: 0; background: var(--paper);
