import { useDynamicVhCssProperty } from "@app-insights360/@layouts/composable/useDynamicVhCssProperty";
import { config } from "@app-insights360/@layouts/config";
import { ContentWidth } from "@app-insights360/@layouts/enums";
import { useLayouts } from "@app-insights360/@layouts";

const { _setAppDir } = useLayouts();

// 🔌 Plugin
export const createLayouts = (userConfig) => {
  const localStorageIsRtl = localStorage.getItem(
    `${userConfig.app.title}-isRtl`
  );
  ...
  const localStorageContentWidth = (() => {
    const storageValue = localStorage.getItem(
      `${userConfig.app.title}-contentWidth`
    );

    return Object.values(ContentWidth).find((v) => v === storageValue);
  })();

  const localStorageNavbarBlur = localStorage.getItem(
    `${userConfig.app.title}-navbarBlur`
  );

  config.app.title = userConfig.app.title;
  config.app.logo = userConfig.app.logo;
  config.app.contentWidth.value =
    localStorageContentWidth || userConfig.app.contentWidth;
  ...
  config.app.isRtl.value = localStorageIsRtl
    ? JSON.parse(localStorageIsRtl)
    : userConfig.app.isRtl;
  ...
  config.icons.sectionTitlePlaceholder =
    userConfig.icons.sectionTitlePlaceholder;
